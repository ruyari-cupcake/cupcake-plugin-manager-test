...
### 기본 push 대상은 항상 `test2` (테스트서버)

```
git push test2 main         ← 기본. 항상 이것만 사용
git push origin main        ← 금지. 사용자가 "본서버" "origin" "프로덕션"이라고 명시한 경우에만
```

> **⚠️ 본서버(origin) push 시: `test2`와 `origin` 둘 다 push한다.**
> ```
> git push test2 main && git push origin main
> ```

### 절대 규칙

1. **`git push`를 실행할 때 리모트를 생략하지 마라.** 반드시 `git push test2 main` 형태로 리모트를 명시한다.
2. **사용자가 "푸시해", "올려", "배포해"라고만 말하면 → `test2`로 push한다.** 예외 없음.
3. **`origin`에 push하려면 사용자가 현재 대화에서 "본서버", "origin", "프로덕션"이라는 단어를 직접 사용한 경우에만 가능하다.**
4. **origin push 시에는 `test2`와 `origin` 둘 다 push한다.** (`git push test2 main && git push origin main`)
4. **"서버로 올려", "push해", "배포해" 등 대상을 특정하지 않은 표현은 전부 `test2`를 의미한다.**
...
- **push 대상 리모트**: 사용자가 "본서버"/"origin"/"프로덕션"을 명시하지 않은 한 **항상 `test2`**. origin push 시에는 test2와 origin 둘 다 push. 위 최우선 규칙 참조.
