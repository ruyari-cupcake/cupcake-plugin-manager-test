...
describe('cpm-url.config.js — environment branches', () => {
    ...
    it('resolves to test URL when CPM_ENV is empty', async () => {
        const origEnv = process.env.CPM_ENV;
        delete process.env.CPM_ENV;
        try {
            const mod = await import('../src/cpm-url.config.js');
            expect(mod.CPM_BASE_URL).toBe('https://cupcake-plugin-manager-test.vercel.app');
    ...
    it('resolves to test URL when CPM_ENV is some unknown value', async () => {
        const origEnv = process.env.CPM_ENV;
